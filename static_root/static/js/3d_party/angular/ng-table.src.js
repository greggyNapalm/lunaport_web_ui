      ...
      compile: function(element, attrs) {
        ...
        angular.forEach(element.find("td"), function(item) {
          ...
          return columns.push({
            ...
            num_cell: ((el.attr('class') == 'num_cell') ? true : false)
          });
        });
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/header.html', '<tr><th ng-class="{sortable: column.sortable,\'sort-asc\': params.sorting[column.sortable]==\'asc\', \'sort-desc\': params.sorting[column.sortable]==\'desc\'}" ng-click="sortBy(column)" ng-repeat="column in columns" ng-show="column.show(this)" class="header"><div ng-class="{num_cell: column.num_cell}">{{column.title}}</div></th></tr><tr ng-show="show_filter"><th ng-repeat="column in columns" ng-show="column.show(this)" class="filter"><form ng-submit="doFilter()"><input type="submit" style="position: absolute; left: -9999px; width: 1px; height: 1px;"/><div ng-repeat="(name, filter) in column.filter"><div ng-include="\'ng-table/filters/\' + filter + \'.html\'"></div></div></form></th></tr>');
	$templateCache.put('ng-table/pager.html', '<div class="pagination ng-cloak"><ul class="pagination"><li ng-class="{\'disabled\': !page.active}" ng-repeat="page in pages" ng-switch="page.type"><a ng-switch-when="prev" ng-click="goToPage(page.number)" >«</a><a ng-switch-when="first" ng-click="goToPage(page.number)" >{{page.number}}</a><a ng-switch-when="page" ng-click="goToPage(page.number)" >{{page.number}}</a><a ng-switch-when="more" ng-click="goToPage(page.number)" >…</a><a ng-switch-when="last" ng-click="goToPage(page.number)" >{{page.number}}</a><a ng-switch-when="next" ng-click="goToPage(page.number)" >»</a></li></ul><div ng-show="params.counts.length" class="btn-group pull-right"><button ng-repeat="count in params.counts" type="button" ng-class="{\'active\':params.count==count}" ng-click="changeCount(count)" class="btn btn-mini">{{count}}</button></div></div>');
}]);
